...
async function aprovarContrato({ nomeArquivo, signatario, dados }) {
  try {
    ...
    // 🔍 Verifica e extrai o UUID corretamente
    const documentKey =
      uploadResponse.data.documents?.[0]?.uuid ||
      uploadResponse.data.uuid ||
      null;

    if (!documentKey) {
      console.log('📤 Resposta completa da D4Sign:', JSON.stringify(uploadResponse.data, null, 2));
